...
def add_arguments(parser):
    ...
    parser.add_argument(
        "-r",
        "--reduced_image_size",
        type=check_reduced_image_size,
        default=32,
        help="Default is 32, ~128 needed to read speech bubbles",
    )
...
def run(args):
    ...
    for name, planet_config in planet_configs.items():
        ...
        planet_tile_directory = pull_images.pull_all_images(
            name, planet_path, width_in_images, args.offline
        )
        ...
        planet_config["loc"] = [
            planet_config["loc"][0] * 2,
            -planet_config["loc"][1] * 2,
        ]
