...
def stitch_planet(max_cord: int, directory: Path, out_directory: Path, planet: str, reduced_image_size: int):
    ...
    planet_image = Image.new("LA", (reduced_image_size * max_cord, reduced_image_size * max_cord))
...
def add_great_attractor_annotation(attractor_width, attractor_height, space_image, reduced_image_size):
    ...
    attractor_annotation = Image.new("LA", ((attractor_width + extra_width_for_text)*1024, attractor_height * 1024), (0, 255))
    ...
    for x in range(space_image.size[0]):
        for y in range(space_image.size[1]):
            ...
            if pixels_so_far % 100000 == 0:
                ...
                print(f"{pixels_so_far} out of {total_amount_pixels}, {percent_complete}% converting to grey complete")
            (g, a) = pixels[x,y]
