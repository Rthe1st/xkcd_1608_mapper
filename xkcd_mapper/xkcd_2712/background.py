...
def black_and_white(grav: float, global_max: float, global_min: float):
    ...
    high_filtered = (min(1, as_percent + high_filtered_bound) - high_filtered_bound) / (
        1 - high_filtered_bound
    )
...
def create_gravity_background_image(
    ...
    space_image = Image.new(
        "LA",
        (reduced_image_size * width_in_tiles, reduced_image_size * height_in_tiles),
    )
            ...
            for y in range(space_image.size[1], 4):
                if calculate_with_x_y_components:
                    strength = gravity_strength(
                        planet_conf, min_x + x * unscale_x, min_y + y * unscale_y
                    )
                else:
                    strength = gravity_strength_no_direction(
                        planet_conf, min_x + x * unscale_x, min_y + y * unscale_y
                    )
    ...
    for x in range(space_image.size[0]):
        for y in range(space_image.size[1]):
            ...
            if pixels_so_far % 100000 == 0:
                ...
                print(
                    f"{pixels_so_far} out of {total_amount_pixels}, {percent_complete}% gravity pixels complete"
                )
            strength = gravity_strength(
                planet_conf, min_x + x * unscale_x, min_y + y * unscale_y
            )
...
def create_star_background_image(
    width_in_tiles: int,
    height_in_tiles: int,
    reduced_image_size: int,
    out_directory: Path,
    ...
    for y in range(height_in_tiles):
        for x in range(width_in_tiles):
            space_image.paste(
                image_tile, (x * background_tile_size, y * background_tile_size)
            )
    image_tile.close()
    return space_image
