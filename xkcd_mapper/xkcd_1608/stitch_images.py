...
def file_name(x, y):
    ...
    default_file_name = f"{os.path.dirname(__file__)}/default_img.png"
...
def stitch_rows_to_image(x_min, x_max, y_min, y_max, reduced_image_size):
    ...
    new_image = Image.new(
        "L", (reduced_image_size * row_width, reduced_image_size * column_height)
    )
    for y in range(y_min, y_max):
        ...
        row_image.close()
    new_image.save(
        "./results/xkcd_1608/whole_picture/total_img_%s_pixel.png" % reduced_image_size
    )
    new_image.close()
